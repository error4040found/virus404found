...
function renderKPIs(t) {
    ...
    el('rev-total').textContent    = '$' + money(t.combined_revenue || t.revenue);
    ...
    makeChart('chart-revenue', {
        ...
        data: {
            ...
            datasets: [
                ...
                {
                    label: 'EXL Revenue',
                    data: daily.map(d => d.exl_revenue || 0),
    ...
    makeChart('chart-sales', {
        ...
        data: {
            ...
            datasets: [
                ...
                {
                    label: 'EXL Sales',
                    data: daily.map(d => d.exl_sales || 0),
                    backgroundColor: C.orangeBg,
                    borderColor: C.orange,
                    borderWidth: 1,
                    borderRadius: 6,
...
function renderDomainRevenue(domains) {
    ...
    const leTotal = leVals.reduce((a,b) => a+b, 0);
    const exlTotal = exlVals.reduce((a,b) => a+b, 0);
    names.push('Grand Total');
    leVals.push(parseFloat(leTotal.toFixed(2)));
    exlVals.push(parseFloat(exlTotal.toFixed(2)));
